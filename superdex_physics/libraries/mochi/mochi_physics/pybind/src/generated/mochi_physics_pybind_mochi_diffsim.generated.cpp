...
void mochi::DefineMochiPhysics_MochiDiffsim([[maybe_unused]] py::module_& m, [[maybe_unused]] PybindRegistry& registry) {
  ...
  registry.GetClass<mochi::diffsim::BackPropagationSceneStats>()
    ...
    .def_readwrite("residual_norm", &mochi::diffsim::BackPropagationSceneStats::residualNorm, "Final residual norm of the adjoint solve in the last back-propagation step: the true residual ||H z - rhs|| of the returned solution (one extra Hessian-vector product), not the solver's internal estimate.\n\nWith :attr:`~superdex.physics.diffsim.BackPropagationSolverParams.validate_finite_diff`\nset, this is the true residual ``|H z - rhs|`` of the returned solution, recomputed\nwith a fresh Hessian-vector product (MINRES's implicit residual can under-report);\notherwise the solver's own estimate.")
  ...
  ;
    ...
    m_diffsim.def("set_soft_material_params_backward", [](mochi::Actor const* actor, mochi::Span<mochi::real> out_grad_material_params) {
      mochi::Error error;
      mochi::diffsim::SetSoftMaterialParamsBackward(actor, out_grad_material_params, error);
      if (!error.IsOK()) {
        throw MochiErrorException(error);
      }
    }
      , py::arg("actor")
      , py::arg("out_grad_material_params")
    ...
    m_diffsim.def("get_displacements_backward", [](mochi::Actor* actor, mochi::Span<mochi::real const> grad_output) {
      mochi::Error error;
      mochi::diffsim::GetDisplacementsBackward(actor, grad_output, error);
      if (!error.IsOK()) {
        throw MochiErrorException(error);
      }
    }
    ...
    m_diffsim.def("set_node_velocities_local_backward", [](mochi::Actor const* actor, mochi::Span<mochi::real> out_grad_velocities) {
      mochi::Error error;
      mochi::diffsim::SetNodeVelocitiesLocalBackward(actor, out_grad_velocities, error);
      if (!error.IsOK()) {
        throw MochiErrorException(error);
      }
    }
