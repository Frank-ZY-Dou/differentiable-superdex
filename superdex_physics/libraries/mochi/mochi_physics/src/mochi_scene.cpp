    ...
    _registry.view<CIslandDescendants const>().each([&](CIslandDescendants const& descendants) {
      ...
      forEachIslandActor([&](entt::entity deformable) {
        ...
        forEachIslandActor([&](entt::entity other) {
          ...
          // stage-start and current SDF Hessians and the Jacobians of its stage-start mapping;
          // a point-cloud collider (shells, rods) has no SDF Hessians, and a ROM's rigid
          // transform layer has no stage-start contact Jacobians.
          MOCHI_ERROR_IF(
              (colliderType(deformable) == ColliderType::PointCloud ||
               _registry.any_of<TagRomActor>(deformable)) &&
                  colliderType(deformable) != ColliderType::None &&
                  _registry.all_of<TagUseContact>(other) && contactEnabled(other, deformable),
              error,
              "Deformable contact adjoints cover SDF colliders only: a differentiable shell, "
              "rod or ROM actor is a point-cloud or ROM collider of a dynamic actor of its "
              "island (no stage-start SDF Hessians or contact Jacobians). Set the deformable's "
              "collider type to NONE (its own samples still detect contact) or disable contact "
              "between them with contact layers or enable_actor_contact.");
          ...
          MOCHI_ERROR_IF(
              otherType == ColliderType::PointCloud || _registry.any_of<TagRomActor>(other),
              error,
              "Deformable contact adjoints against a dynamic collider need its stage-start SDF "
              "Hessians and contact Jacobians (sphere, box, plane, grid-SDF and soft-body SDF "
              "colliders): a differentiable soft or rod actor may contact a point-cloud or ROM "
