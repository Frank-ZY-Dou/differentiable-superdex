...
def _has_bot_task_prefab_loader() -> bool:
    try:
        import superdex.robotics as robotics
    except ImportError:
        return False
    return hasattr(robotics, "load_bot_task_prefab_from_file")


def _has_prefab_test_shapes() -> bool:
    root = pathlib.Path(str(get_assets_root()))
    return all(
        (root / shape).is_file()
        for shape in ("duck/duck_coarse_mesh.mochi.json", "cube/cube_minimal.mochi.json")
    )


@unittest.skipUnless(
    _has_prefab_test_shapes(),
    "the shape descriptions these prefabs reference are not in the open-source asset export",
)
...
@unittest.skipUnless(
    _has_bot_task_prefab_loader(),
    "the open-source superdex_robotics extension does not export load_bot_task_prefab_from_file",
)
