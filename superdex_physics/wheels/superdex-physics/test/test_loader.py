...
class LoaderTest(unittest.TestCase):
    ...
    def test_import_module_names_the_robotics_fp64_extra(self) -> None:
        # Earlier tests (and the scene-helper tests' robotics probe) may have imported the
        # native robotics module; the loader returns a cached module from sys.modules, so
        # isolate this test from that state.
        with patch.dict(sys.modules):
            for cached in ("superdex_robotics_double", "superdex_robotics"):
                sys.modules.pop(cached, None)
            with tempfile.TemporaryDirectory() as temp_dir:
                package_dir = Path(temp_dir) / "superdex" / "physics"
